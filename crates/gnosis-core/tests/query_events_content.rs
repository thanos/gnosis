...
use gnosis_core::{LimitedReader, ScanConfig};
use std::io::Cursor;
...
fn query_summary_objects_unknown_find_explain_graph() {
    ...
    match q.explain("Alpha").unwrap() {
        gnosis_core::ExplainResult::Entity {
            entity,
            neighborhood,
        } => {
    ...
    assert!(matches!(
        ...
        Some(gnosis_core::ExplainResult::Object { .. })
    ));
...
fn ids_helpers() {
    ...
    assert_eq!(gnosis_core::ids::random_suffix().len(), 8);
}
